...
public interface BugRepo extends JpaRepository<Bug, Long> {
    ...
    @Query("SELECT b FROM Bug b WHERE (:projectId IS NULL OR b.projectId = ?1) AND (:status IS NULL OR b.status = ?2) AND (:assigneeId IS NULL OR b.assigneeId = ?3) AND (:priority IS NULL OR b.priority = ?4) AND (:startDate IS NULL OR b.startDate >= ?5) AND (:endDate IS NULL OR b.endDate <= ?6)")
